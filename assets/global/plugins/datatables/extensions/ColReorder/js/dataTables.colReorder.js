...
/**
 * Switch the key value pairing of an new array to be value key (i.e. the old value is now the
...
$.fn.dataTableExt.oApi.fnColReorder = function ( oSettings, iFrom, iTo )
{
	...
	{
		this.oApi._fnLog( oSettings, 1, "ColReorder 'from' new is out of bounds: "+iFrom );
		return;
	}

	if ( iTo < 0 || iTo >= iCols )
	{
		this.oApi._fnLog( oSettings, 1, "ColReorder 'to' new is out of bounds: "+iTo );
		return;
	}

	/*
	 * Calculate the new column array new, so we have a mapping between the old and new
	 */
	var aiMapping = [];
	...
	{
		/* Calculate the current visible new and the point to insert the node before. The insert
...
ColReorder.prototype = {
	...
	"_fnMouseDown": function ( e, nTh )
	{
		...
		var idx = parseInt( $(nTh).attr('data-column-new'), 10 );
	...
	"_fnDestroy": function ()
	{
		...
		$.each( this.s.dt.aoColumns, function (i, column) {
			$(column.nTh).removeAttr('data-column-new');
		} );
	...
	/**
	 * Add a data attribute to the column headers, so we know the new of
	 * the row to be reordered. This allows fast detection of the new, and
	...
	{
		$.each( this.s.dt.aoColumns, function (i, column) {
			$(column.nTh).attr('data-column-new', i);
		} );
	}
};
